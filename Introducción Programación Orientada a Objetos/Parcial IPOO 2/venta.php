...
class Venta {
    ...
    /**
     * Retorna la sumatoria del precio venta de cada una de las bicicletas Nacionales vinculadas a la venta.
     * @return int
     */
    public function retornarTotalVentaNacional() {
        $coleBicis = $this -> getColeccionBicicletas();
        $precioFinal = 0;
        foreach ($coleBicis as $bici) {
            if ($bici instanceof BicicletasNacionales) {
                $precioBici = $bici -> darPrecioVenta();
                $precioFinal = $precioFinal + $precioBici;
            }
        }
        return $precioFinal;
    }
    ...
    /**
     * Retorna una colección de bicicletas importadas vinculadas a la venta. Si la venta solo se corresponde 
     * con bicicletas Nacionales la colección retornada debe ser vacía.
     * @return array
     */
    public function retornarBicicletasImportadas() {
        ...
        $bicisImportadas = [];
        foreach ($coleBicis as $bici) {
            if ($bici instanceof BicicletasImportadas) {
                array_push($bicisImportadas, $bici);
            }
        }
        return $bicisImportadas;
    }
